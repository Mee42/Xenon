...
fun reshuffleExpressions(ast: AST): AST {
    ...
    return AST(ast.functions.map {
        if(it is XenonFunction) {
            it.copy(inst.reshuffle(it.content))
        } else it
    })
}
...
private class Reshuffler(val ast: AST) {
    ...
    fun Expression.reshuffle(): Expression { return when (this) {
        ...
        is MathExpression -> {
            val x = if(var1 is MathExpression && var1.mathType == MathType.ADD &&
                    var2 is MathExpression && var2.mathType == MathType.ADD) {
                ...
                val integerExpression = integerValues.reduce { a: Expression, b: Expression -> MathExpression(a,b,MathType.ADD) }
                val otherFolded = when {
                    other.size == 1 -> other.first()
                    other.isEmpty() -> null
                    else -> other.reduce { a, b -> MathExpression(a,b,MathType.ADD) }
                }
                if(otherFolded == null) integerExpression
                else  MathExpression(otherFolded.reshuffle(), integerExpression.reshuffle(), MathType.ADD)
            } else if(var1.isPure() || var2.isPure()) {
                if(var1 is MathExpression || var1 is EqualsExpression || var1 is IntegerValueExpression) {
                    // it goes second
                    MathExpression(var2.reshuffle(), var1.reshuffle(), mathType)
                } else null
            } else null
            x ?: MathExpression(var1.reshuffle(), var2.reshuffle(), mathType)
        }
        is EqualsExpression -> {
